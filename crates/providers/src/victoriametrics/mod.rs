//! `VictoriaMetrics` / Prometheus-compatible metric provider.
//!
...
/// VM → obz model conversion functions.
pub(crate) mod convert;
/// VM API response deserialization types.
...
/// Factory function: build a [`BuiltProvider`] for `VictoriaMetrics`.
fn build(config: &obz_core::provider::ProviderConfig) -> Result<BuiltProvider, ObzError> {
    ...
    Ok(BuiltProvider {
        ...
        metric_query_language: Some("MetricsQL"),
...
/// Return the [`ProviderMeta`] for `VictoriaMetrics`.
///
