...
//!
//! Jaeger uses the standard Jaeger HTTP API response format.
...
mod tests {
    ...
    /// Load the `response.body` from a Jaeger fixture file and parse as Jaeger response.
    fn load_fixture(name: &str) -> Vec<JaegerTrace> {
        let manifest = env!("CARGO_MANIFEST_DIR");
        let workspace = manifest.trim_end_matches("crates/providers");
        let path = format!("{workspace}fixtures/jaeger/traces/{name}.json");
        let raw =
    ...
    fn fixture_search_by_error() {
        ...
        // The Jaeger fixture for error search returned empty results from the
        // live recording. Verify conversion handles empty data gracefully.
        let result = convert_search_result(traces);
        assert_eq!(result.spans.len(), 0);
        assert_eq!(result.total_count, 0);
    }
    ...
    #[test]
    fn fixture_services() {
        let services = load_operations_fixture("services");
        assert!(!services.is_empty(), "should have services");
    }
