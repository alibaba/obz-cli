//! `VictoriaTraces` trace provider.
//!
//! Implements the [`TraceProvider`] trait against the Jaeger HTTP Query API
//! exposed by `VictoriaTraces` at `/select/jaeger/api/`.
//!
//! # API Endpoints
//!
//! | obz Command  | Endpoint                            | Response Format |
//! |--------------|-------------------------------------|-----------------|
//! | `trace search` | `GET /select/jaeger/api/traces`   | JSON            |
//! | `trace get`    | `GET /select/jaeger/api/traces/{id}` | JSON         |
...
/// `VictoriaTraces` trace provider (Jaeger-compatible HTTP API).
#[derive(Clone)]
pub(crate) struct VictoriaTracesProvider {
    /// Base URL of the `VictoriaTraces` instance (e.g. `http://localhost:9428`).
    base_url: String,
...
impl VictoriaTracesProvider {
    /// Create a new `VictoriaTracesProvider`.
    ///
    /// # Arguments
    ///
    /// * `base_url` — Base URL of the `VictoriaTraces` instance (no trailing slash).
...
#[async_trait]
impl TraceProvider for VictoriaTracesProvider {
    ...
    async fn get_trace(&self, params: &TraceGetParams) -> ProviderResult<TraceDetail> {
        let req = self
            .get(&format!("/select/jaeger/api/traces/{}", params.trace_id))?
            .query(&[
                ("start", &(params.start * 1_000_000).to_string()),
                ("end", &(params.end * 1_000_000).to_string()),
            ]);
...
#[async_trait]
impl ExtensionProvider for VictoriaTracesProvider {
    async fn execute(
        &self,
        command: &str,
        ...
        match command {
            "services" => {
                let req = self.get("/select/jaeger/api/services")?;
            ...
            "operations" => {
                ...
                let req = self.get(&format!("/select/jaeger/api/services/{service}/operations"))?;
                let items: Vec<String> = self.send_jaeger(req).await?;
                Ok(ExtensionResult::from_strings(items))
            }
            _ => Err(ObzError::Unsupported {
                message: format!("unknown extension command: {command}"),
                provider: Some("victoriatraces".to_string()),
                suggestion: None,
            }),
        }
    }
...
/// Extension commands for `VictoriaTraces`.
...
/// Factory function: build a [`BuiltProvider`] for `VictoriaTraces`.
fn build(config: &obz_core::provider::ProviderConfig) -> Result<BuiltProvider, ObzError> {
    ...
    let provider = VictoriaTracesProvider::new(
        endpoint,
        config.bearer_token(),
        basic,
    ...
    // Clone shares the underlying reqwest::Client (Arc-based connection pool).
...
mod tests {
    ...
    /// Create a VT provider pointed at a dummy URL (for testing error paths
    /// that return before making HTTP calls).
    fn dummy_provider() -> VictoriaTracesProvider {
        // Install ring as the crypto provider for tests (reqwest needs it
        // when built with rustls-no-provider).
        let _ = rustls::crypto::ring::default_provider().install_default();
        VictoriaTracesProvider::new(
            "http://localhost:0",
            None,
            None,
    ...
    async fn extension_unknown_command() {
        ...
        match result.unwrap_err() {
            ObzError::Unsupported {
                ...
                assert_eq!(provider.as_deref(), Some("victoriatraces"));
            }
